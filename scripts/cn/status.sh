...
function status()
{
	...
	while true; do
		...
		local kusama_head_block=$(node $installdir/scripts/console.js --substrate-ws-endpoint "wss://kusama.api.onfinality.io/public-ws" chain sync-state | awk -F " " '/currentBlock/ {print $NF}' | sed 's/,//g')
